...
taysi_osoite2 = '/root/paja/kahvi/taysi_backup.txt'  # milloin kahvi keitetty
...
while 1:
    ...
    currentTime = (today.strftime('%m/%d/%Y ') +
    ...
    if (i == 0):
        ...
        with open(taysi_osoite2, 'r') as fkirj3:
            ...
            aika = (int(datetime.datetime.now().strftime("%H"))*60 +
                    int(datetime.datetime.now().strftime("%M")))
    # Onko laite usb-portissa
    try:
        ser = serial.Serial(portti, baudrate=9600, timeout=4,
                            rtscts=0, dsrdtr=0)
    ...
    if (aika % 5 == 0):  # 5min valein
        if (csv_lahetetty == 0):
            # os.system('cat /root/paja/kahvi/kahvidata.csv ' +
            #           '| ssh kahvi@sika "cat >> /home/kahvi/kahvidata.csv"')
            # os.system('scp "%s" "kahvi@sika:"' % (tallennusosoite))
            os.system('rsync --append "%s" "kahvi@sika:"' % (tallennusosoite))
            os.system('ssh kahvi@sika "tail -n 2000 kahvidata.csv ' +
                      '> kahvidatatail.csv"')
            csv_lahetetty = 1
    else:
        csv_lahetetty = 0
    ...
    if (aika % 2 == 0):
        ...
        if (status == 5):
            keittoaika = (today.strftime('%d.%m.') + ' klo ' +
        ...
        while(1):
            ...
            if (minuuttierotus < 180):
                ...
                break
            if (minuuttierotus > 180):
                pannutieto = ('Kahvi keitetty monta tuntia sitten, ' +
                              'lienee jo loppu')
                break
            break
    else:
